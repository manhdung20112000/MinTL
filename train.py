...
class Model(object):
    ...
    def train(self):
        ...
        for epoch in tqdm.tqdm(range(cfg.epoch_num), desc="Epoch"):
            log_loss = 0
            log_dst = 0
            log_resp = 0
            log_cnt = 0
            ...
            logging.info(
                'epoch: %d, train loss: %.3f, valid loss: %.3f, total time: %.1fmin' % (epoch + 1, epoch_sup_loss,
                                                                                        valid_loss,
                                                                                        (time.time() - sw) / 60))
        ...
        for batch_num, dial_batch in tqdm.tqdm(enumerate(data_iterator),desc="Evaluate"):
            ...
            for turn_num, turn_batch in enumerate(dial_batch):
                ...
                if self.args.noupdate_dst:
                    ...
                    dst_outputs, resp_outputs = self.model.inference(tokenizer=self.tokenizer, reader=self.reader,
                                                                     prev=py_prev, input_ids=inputs['input_ids'],
                                                                     attention_mask=inputs["masks"],
                                                                     turn_domain=inputs["turn_domain"],
                                                                     db=inputs["input_pointer"])
...
def main():
    ...
    if args.mode == 'test' or args.mode == 'relex':
        ...
        parse_arg_cfg(args)
        if args.model_path == "":
            args.model_path = 'experiments/{}_sd{}_lr{}_bs{}_sp{}_dc{}_cw{}_model_{}_noupdate{}_{}/'.format(
